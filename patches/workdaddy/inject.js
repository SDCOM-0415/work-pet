 ...
 */
// [WorkPet 兼容垫片] CodeBuddy（VS Code fork）环境实测结论：
//  - 开启了 Trusted Types 强制（require-trusted-types-for），无 default policy：
//    普通 innerHTML/insertAdjacentHTML 赋字符串被直接拒绝；
//  - createPolicy 受 CSP 允许清单限制：随机名被拒；宿主自己会创建的名字（richScreenReaderContent、
//    dompurify、amdLoader、notebookRenderer 等）不能抢注，否则宿主随后 createPolicy 同名即崩；
//    宿主代码全量检索确认 notebookChatEditController 仅存在于 CSP 清单、JS 里 0 次引用 → 可安全占用；
//  - DOMParser.parseFromString 与独立 createHTMLDocument 的 innerHTML 均受 TT 管制，不可作解析兜底。
// 解法：创建「宿主永不使用」的宽松 policy，把字符串经 policy 包成 TrustedHTML 后再走原生 sink。
// WorkBuddy（未开启 TT）上 createPolicy 要么成功（无害）要么失败（policy=null 原样赋值，原生路径）。
(function () {
  try {
    var policy = null;
    if (window.trustedTypes && window.trustedTypes.createPolicy) {
      try { policy = window.trustedTypes.createPolicy('notebookChatEditController', { createHTML: function (s) { return String(s); } }); } catch (_) {}
    }
    var toHTML = function (v) { return policy ? policy.createHTML(v == null ? '' : String(v)) : v; };
      ...
      if (desc && desc.set) {
        try {
          Object.defineProperty(Element.prototype, prop, {
            get: desc.get,
            set: function (v) { desc.set.call(this, typeof v === 'string' ? toHTML(v) : v); },
            configurable: true,
          });
        } catch (_) {}
      }
    });
    var adjDesc = Object.getOwnPropertyDescriptor(Element.prototype, 'insertAdjacentHTML');
    if (adjDesc && adjDesc.value) {
      try {
        Object.defineProperty(Element.prototype, 'insertAdjacentHTML', {
          value: function (position, v) { adjDesc.value.call(this, position, typeof v === 'string' ? toHTML(v) : v); },
          configurable: true,
        });
      } catch (_) {}
    }
    window.__wbsTTReady = !!policy;
  } catch (_) { window.__wbsTTReady = false; }
})();
...
(function () {
  ...
  function build() {
    ...
    if (accountsPane) {
      ...
      var expBtn = root.querySelector('[data-act="export"]');
      if (expBtn) expBtn.addEventListener('click', onExportAccounts);
      var impBtn = root.querySelector('[data-act="import"]');
      if (impBtn) impBtn.addEventListener('click', function () {
        var f0 = root.querySelector('#wbs-import-file');
        if (f0) f0.click();
      });
      var impFile = root.querySelector('#wbs-import-file');
      if (impFile) impFile.addEventListener('change', onImportFile);
    }
