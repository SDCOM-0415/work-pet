...
const DAEMON_BUILD_ID = 'release-1.0.17-20260824-download-progress-copy';
...
restoreSleepMode();
// 启动即备份一次当前登录：CodeBuddy 这类客户端不启动也能有自己的账号备份，
// 不依赖 CDP 连接（文件监听只在文件变化时触发，冷启动没有初始同步）。
if (AUTH_FILE) {
  try { backupCurrent(DATA_DIR, log); } catch (e) { log(`[sync] 启动备份当前登录失败: ${e.message}`); }
}
startServer();
cdpLoop();
